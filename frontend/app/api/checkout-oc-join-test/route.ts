...
export async function GET(request: NextRequest) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      discounts: [
        {
          promotion_code: OC_JOIN_TEST_PROMO_ID,
        },
      ],
      success_url: `${origin}/vibe-coding/oc-join?success=true`,
    ...
    console.error('Error creating Stripe Checkout session for OC Join test promo', error)
    return NextResponse.json(
      {
